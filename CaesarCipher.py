def encrypt(PlainText, shift_value):
    ...
    for i in PlainText:
        if i.isalpha():  # Check if the character is a letter
            if i.islower():
                Cy_text += chr((ord(i) - ord('a') + shift_value) % 26 + ord('a')) # shift each character by the value of shift_value
            elif i.isupper():
                Cy_text += chr((ord(i) - ord('A') + shift_value) % 26 + ord('A'))
        else:
...
def decrypt(cipherText, shift_value):
    ...
    for i in cipherText:
        if i.isalpha():
            if i.islower():
                Pl_Text += chr((ord(i) - ord('a') - shift_value) % 26 + ord('a'))
            elif i.isupper():
                Pl_Text += chr((ord(i) - ord('A') - shift_value) % 26 + ord('A'))
        else:
            Pl_Text += i
